...
class HoppingTab(ttk.Frame):
    """Tab cho Hopping Automation với UI cải tiến."""

    def __init__(self, parent, agent: Agent):
        super().__init__(parent)
        self.agent = agent
        self.hopping_automation: Optional[HoppingAutomation] = None
    ...
    def setup_ui(self):
        """Thiết lập giao diện cho Hopping tab."""
        ...
        # 1. Hop Configuration
        config_section = ttk.LabelFrame(left_frame, text="🌍 Hopping Settings", padding=10)
        ...
        # Snapshot directory
        self.snapshot_dir_var = tk.StringVar(value="./result/hopping/snapshots")
        ...
        # Results directory
        self.results_dir_var = tk.StringVar(value="./result/hopping/results")
        ...
        ttk.Label(settings_inner, text="Loading Wait:", font=('', 10)).grid(row=4, column=0, sticky='w', pady=2)
        loading_frame = ttk.Frame(settings_inner)
        loading_frame.grid(row=4, column=1, sticky='ew', padx=5, pady=2)
    ...
    def get_config(self) -> Dict[str, Any]:
        """Lấy cấu hình từ UI."""
        try:
            num_hops = int(self.num_hops_var.get())
            wait_time = float(self.wait_time_var.get())
            loading_wait = float(self.loading_wait_var.get())
        except ValueError:
            num_hops = 5
            wait_time = 1.0
            loading_wait = 5.0
    ...
    def start_automation(self):
        ...
        self.status_var.set("🌍 Running hopping automation...")
        ...
        logger.info("Hopping automation started")

    def _run_automation(self, config: Dict[str, Any]):
        """Chạy automation (trong thread)."""
        try:
            # Khởi tạo HoppingAutomation
            self.hopping_automation = HoppingAutomation(self.agent, config)

            # Run hopping
            success = self.hopping_automation.run(config)
    ...
    def _automation_finished(self, success: bool, error_msg: str = ""):
        ...
        if success:
            self.status_var.set("✅ Hopping automation completed!")
            self.results_var.set("Check results folder for detailed statistics")
            messagebox.showinfo("Success", "Hopping automation completed!")
        else:
            self.status_var.set("❌ Hopping automation failed")
            self.results_var.set("Failed - check logs for details")
            msg = "Hopping automation failed!"
            if error_msg:
                msg += f"\n\nError: {error_msg}"
    ...
    def stop_automation(self):
        ...
        if messagebox.askyesno("Confirm", "Are you sure you want to stop?"):
            ...
            logger.warning("Hopping automation stopped by user")
